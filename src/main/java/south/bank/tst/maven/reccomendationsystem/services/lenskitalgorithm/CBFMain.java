...
import south.bank.tst.maven.reccomendationsystem.services.lenskitalgorithm.dao.*;
...
public class CBFMain {
    ...
    public static void main(String[] args) throws RecommenderBuildException {
        args = new String[]{"runecbf", "1893439518350526090"};
        ...
        if (args.length == 0) {
             System.out.println("No users specified; provide user IDs as command line arguments");
        }
            ...
            for (String user: args) {
                ...
                try {
                    uid = Long.parseLong(user);
                     ...
                     System.out.println("cannot parse user {} " + user);
                    continue;
                }
                 System.out.println("searching for recommendations for user {}" + user);
                List<ScoredId> recs = irec.recommend(uid, 5);
                if (recs.isEmpty()) {
                    System.out.println("no recommendations for user {}, do they exist? " + uid);
                }
    ...
    public synchronized static List<Long> generateRecommendationsForUser(long uid) throws RecommenderBuildException {
        final List<Long> recommendationsIds = new ArrayList<>();
        LenskitConfiguration config = configureRecommender();
        ...
        try {
            // Generate 5 recommendations for each user
             ...
             System.out.format("recommendations for user %d:\n", uid);
             for (ScoredId id: recs) {
                 recommendationsIds.add(id.getId());
                 System.out.format("  %d: %.4f\n", id.getId(), id.getScore());
             }
        } catch (UnsupportedOperationException e) {
            if (e.getMessage().equals("stub implementation")) {
    ...
    private static LenskitConfiguration configureRecommender() {
        ...
        config.set(TagFile.class)
              .to(new File("data/rss-tags.csv"));
        // and title file
        config.set(TitleFile.class)
              .to(new File("data/rss-urls.csv"));
